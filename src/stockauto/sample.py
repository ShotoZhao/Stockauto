...
from stock_index import INDEX_SYMBOL
from stock_mongo import StockToMongo
import time
...
def _example_stock_quote(quote_ctx):
    ...
    """
    stock_code_list = ["SZ.000687", "SZ.000709", "SZ.002449"]
...
def _example_hist_kline(quote_ctx):
    sub_type = 'K_DAY'
    for i in range(3500):
        try:
            code = INDEX_SYMBOL[i]
            ret_code, ret_data = quote_ctx.get_history_kline(code, start="2017-08-13", ktype=sub_type, autype='qfq')
            ...
            # stockdata = pd.DataFrame(ret_data)
            # stockdata.to_csv("stockdata.csv",index=code)
            # print  ret_data.shape
            print("%s KLINE %s" % (code, sub_type))
            # print(kline_table)
            print i
            print("\n\n")
            cur = StockToMongo("localhost", 27017, "test", "stock_price")
            cur.kline_tomongo(kline_table)
        except Exception, e:
            print e
...
def _example_cur_kline(quote_ctx):
    ...
    """
    # subscribe Kline
    # stock_code_list = ["SZ.300676"]
    # sub_type_list = ["K_1M", "K_5M", "K_15M", "K_30M", "K_60M", "K_DAY", "K_WEEK", "K_MON"]
    sub_type = 'K_DAY'
    for j in range(0, 3600, 250):
        for i in range(j, j + 250):
            code = INDEX_SYMBOL[i]
        ...
        for i in range(j, j + 250):
            code = INDEX_SYMBOL[i]
            ret_code, ret_data = quote_ctx.get_cur_kline(code, 5, sub_type)
            if ret_code == RET_ERROR:
                print(code, sub_type, ret_data)
                exit()
            kline_table = ret_data
            # stockdata = pd.DataFrame(ret_data)
            # stockdata.to_csv("stockdata.csv",index=code)
        ...
        time.sleep(65)
        for n in range(j, j + 250):
            code = INDEX_SYMBOL[n]
            print n

            ret_code, ret_data = quote_ctx.unsubscribe(code, sub_type, unpush=False)
...
def _example_stock_basic(quote_ctx):
    ...
    """
    ret_status, ret_data = quote_ctx.get_stock_basicinfo("SH", "STOCK")
    if ret_status == RET_ERROR:
        print(ret_data)
        exit()
    print("stock_basic")
    # print(ret_data)
    cur = StockToMongo("localhost", 27017, "test", "stockBasic")
    cur.basic_tomongo(ret_data)
...
if __name__ == "__main__":
    ...
    #
    # 获取推送数据
    # quote_context.subscribe('SZ.000687', "QUOTE", push=True)
    ...
    # _example_stock_quote(quote_context)
    _example_hist_kline(quote_context)
